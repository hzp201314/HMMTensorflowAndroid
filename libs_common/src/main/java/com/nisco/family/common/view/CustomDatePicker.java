...
public class CustomDatePicker {
    ...
    public CustomDatePicker(Context context, ResultHandler resultHandler, String startDate, String endDate, int flag) {
        canAccess = true;
    ...
    private void initDialog() {
        if (datePickerDialog == null) {
            datePickerDialog = new Dialog(context, R.style.common_time_dialog);
            datePickerDialog.setCancelable(false);
            datePickerDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
            datePickerDialog.setContentView(R.layout.common_custom_datepicker);
    ...
    private void initView() {
        ...
        TextView day_tv = (TextView) datePickerDialog.findViewById(R.id.day_tv);
        if (flag == 1) {
...
//                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.CHINA);
    ...
    private void dayChange() {
        ...
        int len = selectedCalender.getActualMaximum(Calendar.DAY_OF_MONTH);
        if (selectedYear == startYear && selectedMonth == startMonth) {
            for (int i = startDay; i <= len; i++) {
                day.add(formatTimeUnit(i));
            }
...
//            SimpleDateFormat format = new SimpleDateFormat(template, Locale.CHINA);
    ...
    public void showDate2(String input) {

        if (canAccess) {
            if("开始日期".equals( input )||"截止日期".equals( input )){
                // 指定日期格式
//            SimpleDateFormat format = new SimpleDateFormat(template, Locale.CHINA);
                SimpleDateFormat   formatter   =   new   SimpleDateFormat   ("yyyy-MM-dd");
                Date curDate =  new Date(System.currentTimeMillis());
            ...
            if (isValidDate(input, "yyyy-MM-dd")) {
                if (startCalendar.getTime().getTime() < endCalendar.getTime().getTime()) {
                    canAccess = true;
                    initParameter();
                    initTimer();
