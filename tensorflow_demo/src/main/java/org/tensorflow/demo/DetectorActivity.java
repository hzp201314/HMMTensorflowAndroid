 ...
 * objects.
 */
@Route(path = "/tensorflow/activity/DetectorActivity")
public class DetectorActivity extends CameraActivity implements OnImageAvailableListener {
    ...
    protected void processImage() {
        ...
        byte[] originalLuminance = getLuminance();//返回yuvBytes[0]
        tracker.onFrame(
                previewWidth,
                previewHeight,
                getLuminanceStride(),//步长:大于等于previewWidth
                sensorOrientation,
                originalLuminance,
                timestamp );
        /**
         * 在事件循环的后续循环中导致失效。使用此选项可使非UI线程中的视图无效。
         * 仅当此视图附加到窗口时可以从UI线程外部调用此方法
         */
        trackingOverlay.postInvalidate();

//    不需要互斥，因为此方法不可重入。首次computingDetection=flase
        // No mutex needed as this method is not reentrant.
        if (computingDetection) {
            readyForNextImage();//准备下次图片
            return;
        }
//      计算机检测标识，true为检测完毕，准备进入下一次检测的标识
        ...
        if (luminanceCopy == null) {
            luminanceCopy = new byte[originalLuminance.length];//新建一个图像亮度字节数组
        }
        ...
        if (SAVE_PREVIEW_BITMAP) {
            ImageUtils.saveBitmap( croppedBitmap );//保存裁剪位图
        }
        ...
        runInBackground(
                new Runnable() {
                    ...
                    public void run() {
                        ...
                        lastProcessingTimeMs = SystemClock.uptimeMillis() - startTime;//识别耗费时间

                        // 2 将得到的分类的名字及概率等信息，反馈到app上。也就是results区域
                        ...
                        for (final Classifier.Recognition result : results) {
                            ...
                            if (location != null && result.getConfidence() >= minimumConfidence) {
                                canvas.drawRect( location, paint );//画矩阵及位置

                                /**
                                 * 将此矩阵应用于矩形，并将转换后的矩形写回它。
                                 * 这是通过转换rect的4个角，然后将其设置为那些点
                                 *
                                 * @param rect 要转换的矩形。
                                 * @return 调用rectstatysrect（）的结果
                                 */
                                cropToFrameTransform.mapRect( location );
                                result.setLocation( location );//设置位置
                                mappedRecognitions.add( result );//将result添加到mappedRecognitions列表。
                            }
                        }
    ...
    /**
     * 此方法在CameraActivity中新建CameraFragment布局时填充布局所使用
     *
     * @return 返回布局
     */
    @Override
